...
(defmethod delete-backward ((item resource) (end number))
  (dolist (parent (resource-predecessors item))
    ...
    (if (and (>= (best-start-time parent) end) (< 0 (resource-duration item)))
        (progn
  ...
  (let ((done t) (count 0) cur-trial)
    (do ()
      ...
      (multiple-value-bind (result args) (parse-item p)
        ...
        (if (and (> (current-stack-length) *stack-warn-limit*) (not *warned-about-stack*))
            (progn
                            ...
                            (mapcar
                             #'(lambda (q)
                                 (let ((item (first (last (resource-queue-tree q)))))
                                   (cond 
                                    ((null item) nil)
                                    ((and (>= (best-start-time item) (gethash :end args)) (< 0 (resource-duration item)))
                                     (delete-forward item)
                                     (delete-backward item (gethash :end args))
                                     (dolist (queue (gethash (resource-type item) (processor-queues (get-processor))))
                                       (if (member item (resource-queue-tree queue))
                                           (setf (resource-queue-tree queue) (remove item (resource-queue-tree queue)))))
                                     )
...
(defmethod run-on-alternate-thread (func)
  (mp:process-run-function "Processing Thread" '(:priority -100000)
                           #'(lambda (in out err)
