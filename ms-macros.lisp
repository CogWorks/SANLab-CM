(defmacro megabytes (n)
  (* n 1024 1024))

(defmethod prepare-processor ((p processor))
  (maphash
   #'(lambda (name items)
       (declare (ignore name))
       (loop for i in items do
	     (loop for j from 1
		   to (resource-constraint-count i) do
		   (let ((queue (make-instance 'resource-queue)))
		     (push queue (resource-constraint-queues i))
		     (push queue
			   (gethash (get-activity-by-typename
				     (constraint-type i))
				    (processor-queues p)))))))
   (processor-constraints p)))

(let (the-processor)

(defun set-processor (x) (setf the-processor x))

(defmacro defprocessor (name &body body)
  `(prog ()
     (declare (special ,name))
     (bootstrap)
     (set-processor
      (let ((the-processor (make-instance (quote processor))))
        ,@body
        (prepare-processor the-processor)
        the-processor))
     (setf ,name (get-processor)))
  )

(defun get-processor () the-processor)

)

(defmacro defconstraint (name &rest args
			      &key constraint-type
			      (type 'resource-constraint)
			      &allow-other-keys)
  (let ((args (remove-pair args :constraint-type)))
    `(push (make-instance (quote ,type) 
			  :name ,name ,@args 
			  :type ,constraint-type)
	   (gethash ,name (processor-constraints the-processor)))))

(defun print-if (test &rest args)
  (if test (apply #'format t args)))
