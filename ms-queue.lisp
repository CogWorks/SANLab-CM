...
(defmethod schedule-resource ((item resource) (queue resource-queue))
  (if (equal (resource-type item) (get-activity-by-typename "Cognitive Operator")) (push item *insert-items*))
  (let* ((pos (position queue (flatten (as-list (processor-queues (get-processor)))))))
    (setf (resource-queue-tree queue) (insert item (resource-queue-tree queue) nil))
  ...
  (cond ((null queue) queue)
        ((and (eq method :end)
         ...
         (let ((act1 (car queue))
           ...
           (setf (resource-type act2) (resource-type act1)
                 (resource-start-time act2) (if (valid-time? (resource-start-time act1))
                                                (best-end-time item) (resource-start-time act1))
                 (resource-end-time act2) (resource-end-time act1)
                 (resource-latest-start-time act2) (if (valid-time? (resource-latest-start-time act1))
                                                       (best-end-time item) (resource-latest-start-time act1))
                 (resource-latest-end-time act2) (resource-latest-end-time act1)
                 (resource-earliest-start-time act2) (if (valid-time? (resource-earliest-start-time act1))
                                                         (best-end-time item) (resource-earliest-start-time act1))
                 (resource-earliest-end-time act2) (resource-earliest-end-time act1)
                 (resource-parameters act2) (copy-list (resource-parameters act1))
                 (resource-id act2) (resource-id act1)
                 (resource-iroutine act2) (resource-iroutine act1)
           ...
           (if (valid-time? (resource-latest-end-time act1))
               (setf (resource-latest-end-time act1) (best-end-time item)))
           (if (valid-time? (resource-earliest-end-time act1))
               (setf (resource-earliest-end-time act1) (best-end-time item)))
        ...
        ((and (eq method :start)
         ...
         (let ((act1 (car queue))
           ...
           (setf (resource-type act2) (resource-type act1)
                 (resource-start-time act2) (if (valid-time? (resource-start-time act1))
                                                (best-start-time item) (resource-start-time act1))
                 (resource-end-time act2) (resource-end-time act1)
                 (resource-latest-start-time act2) (if (valid-time? (resource-latest-start-time act1))
                                                       (best-start-time item) (resource-latest-start-time act1))
                 (resource-latest-end-time act2) (resource-latest-end-time act1)
                 (resource-earliest-start-time act2) (if (valid-time? (resource-earliest-start-time act1))
                                                         (best-start-time item) (resource-earliest-start-time act1))
                 (resource-earliest-end-time act2) (resource-earliest-end-time act1)
                 (resource-parameters act2) (copy-list (resource-parameters act1))
                 (resource-id act2) (resource-id act1)
                 (resource-iroutine act2) (resource-iroutine act1)
                 ...
                 (resource-distribution act2) (resource-distribution act1)
                 (resource-dependents act2) (copy-list (resource-dependents act1))
                 (resource-predecessors act2) (list act1))
           (if (valid-time? (resource-end-time act1))
               (setf (resource-end-time act1) (best-start-time item)))
           (if (valid-time? (resource-latest-end-time act1))
               (setf (resource-latest-end-time act1) (best-start-time item)))
           (if (valid-time? (resource-earliest-end-time act1))
               (setf (resource-earliest-end-time act1) (best-start-time item)))
           (compute-duration act1)
           (compute-duration act2)
        ...
        (t
         (setf (cdr queue) (scan-and-split item (cdr queue) :method method :action action))
         queue))
)

(defmethod scan-and-split ((item resource) (queue resource-queue) &key (method :end) (action nil))
  (setf (resource-queue-tree queue) (scan-and-split item (resource-queue-tree queue) :method method :action action))
)

(defmethod schedule-resource :after ((item resource) (queue resource-queue))
  (declare (special *interrupts*))
  (block nil
    (if (not (boundp '*interrupts*)) (return))
    (dolist (entry *interrupts*)
      (destructuring-bind (act1 act2) (mapcar #'get-activity-by-typename entry)
        (if (is-supertype-of? act1 (resource-type item))
            (let (queues)
              (maphash #'(lambda (k v) (if (is-supertype-of? act2 k) (setf queues (append queues v))))
